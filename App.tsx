...
const App: React.FC = () => {
  ...
  const syncToSpreadsheet = async () => {
    ...
    setError(null);
    try {
      // Note: Google Apps Script Web Apps usually require CORS or no-cors
      ...
      setError('Sync failed. Please verify your Google Apps Script URL is correct and deployed as a Web App.');
    } finally { 
      setIsSyncing(false); 
    }
  };
        ...
        {activeTab === 'upload' ? (
          <div className="space-y-8 max-w-5xl mx-auto">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 px-1">Cloud Sync Endpoint</label>
              <input 
                type="text" 
                value={scriptUrl}
                onChange={(e) => setScriptUrl(e.target.value)}
                placeholder="https://script.google.com/macros/s/..."
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs font-mono focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
              />
            </div>
            ...
            <div className={`bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center text-center transition-all ${isProcessing ? 'py-20' : 'py-16'}`}>
              ...
              <p className="text-slate-500 text-sm mb-8 px-6 max-w-md">Securely upload your bank statement PDF. Our AI will automatically extract and categorize every transaction for you.</p>
              ...
              <label className={`cursor-pointer px-10 py-4 rounded-2xl font-bold transition-all shadow-xl flex items-center ${isProcessing ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-slate-800 hover:-translate-y-1'}`}>
                ...
                {isProcessing ? 'Extracting Data...' : 'Choose PDF File'}
              ...
              <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-10 duration-500">
                <div className="px-6 py-5 bg-slate-50/80 border-b border-slate-100 flex justify-between items-center">
                   <div className="flex flex-col">
                     <span className="text-lg font-bold text-slate-800">{transactions.length} Transactions Found</span>
                     <span className="text-xs text-indigo-600 font-semibold">{transactions.filter(t => t.status === 'approved').length} marked for sync</span>
                   </div>
                   <button 
                    onClick={syncToSpreadsheet} 
                    disabled={isSyncing || transactions.filter(t => t.status === 'approved').length === 0}
                    className="bg-emerald-600 text-white px-8 py-3 rounded-2xl text-sm font-bold shadow-lg shadow-emerald-100 hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none transition-all active:scale-95"
                   >
                     {isSyncing ? <><i className="fas fa-spinner fa-spin mr-2"></i> Syncing...</> : 'Approve & Sync'}
                   </button>
                </div>
